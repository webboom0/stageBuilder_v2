          ...
          if (track.uuid) {
            ...
            if (trackData) {
              ...
              if (index !== -1) {
                ...
                // 새 위치에 키프레임 추가
                trackData.addKeyframe(newTimeInSeconds, value, interpolation);
          ...
          if (isOutsideClip) {
            ...
            if (track.uuid) {
              ...
              if (trackData) {
                ...
                console.log("삭제할 키프레임 시간:", currentTime, "속성:", property);
                trackData.removeKeyframe(currentTime);
                console.log(`${property} 속성 키프레임 삭제됨:`, currentTime);
              } else {
      ...
      const handleMouseMove = (e) => {
        ...
        if (relativeX < 0 || relativeX > spriteRect.width) {
          ...
          if (track.uuid) {
            ...
            if (trackData) {
              const oldTime = parseFloat(keyframeElement.dataset.time);
              const index = trackData.times.indexOf(oldTime);
              if (index !== -1) {
                const value = new THREE.Vector3(
                  trackData.values[index * 3],
                  trackData.values[index * 3 + 1],
        ...
        if (isDragging) {
          ...
          if (isOutsideClip) {
            ...
            if (track.uuid) {
              ...
              if (trackData) {
                const currentTime = parseFloat(keyframeElement.dataset.time);
                console.log("삭제할 키프레임 시간:", currentTime, "속성:", propertyType);
                trackData.removeKeyframe(currentTime);
                console.log(`${propertyType} 속성 키프레임 삭제됨:`, currentTime);
              } else {
