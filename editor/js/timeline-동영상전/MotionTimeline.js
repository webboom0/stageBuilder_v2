...
export class MotionTimeline extends BaseTimeline {
  ...
  createBackground() {
    ...
    video.src = "/path/to/your/video.mp4"; // 비디오 파일 경로
    video.loop = true;
    video.muted = true;
    ...
    // 배경 평면 생성
    const geometry = new THREE.PlaneGeometry(16, 9); // 16:9 비율
    const material = new THREE.MeshBasicMaterial({
      map: videoTexture,
      side: THREE.DoubleSide,
    });
    const background = new THREE.Mesh(geometry, material);
    background.position.z = -10; // 카메라 뒤에 위치
    ...
    // Stage 그룹에 추가
    const stageGroup = this.editor.scene.children.find(
