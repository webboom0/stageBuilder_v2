...
export class MotionTimeline extends BaseTimeline {
  ...
  makeKeyframeDraggable(keyframeElement, track, frame, object) {
    ...
    keyframeElement.addEventListener("contextmenu", (e) => {
      ...
      deleteItem.addEventListener("click", () => {
        ...
        if (
          this.selection &&
          this.selection.type === "keyframe" &&
          this.selection.frame === frame
        ) {
          this.selection = {
            type: null,
            objectId: null,
        ...
        console.log("Keyframe deleted:", { frame });
      });
  ...
  bindSpriteEvents(sprite, track) {
    ...
    sprite.addEventListener("mousedown", (e) => {
      // 리사이즈 핸들이나 키프레임 관련 요소를 클릭한 경우
      if (e.target.classList.contains("resize-handle")) {
        return;
      }

      // 키프레임이나 키프레임 레이어를 클릭한 경우
      if (e.target.classList.contains("keyframe")) {
        // 키프레임의 frame 값 가져오기
        const frame = parseInt(e.target.dataset.frame);
        if (!isNaN(frame)) {
          // 키프레임 드래그 시작
          const keyframeStartLeft = parseInt(e.target.style.left) || 0;

          const onKeyframeMove = (moveEvent) => {
            const deltaX = moveEvent.clientX - startX;
            const newLeft = keyframeStartLeft + deltaX;

            // 클립 내에서만 이동하도록 제한
            const constrainedLeft = Math.max(
              0,
              Math.min(newLeft, sprite.offsetWidth)
            );
            e.target.style.left = `${constrainedLeft}px`;
          };

          const onKeyframeUp = () => {
            document.removeEventListener("mousemove", onKeyframeMove);
            document.removeEventListener("mouseup", onKeyframeUp);
          };

          startX = e.clientX;
          document.addEventListener("mousemove", onKeyframeMove);
          document.addEventListener("mouseup", onKeyframeUp);
        }
        return;
      }
      ...
      this.selectClip(sprite, track);

      isDragging = true;
      startX = e.clientX;
      originalLeft = parseInt(sprite.style.left) || 0;
    });

    document.addEventListener("mousemove", (e) => {
      if (!isDragging || !this.selection || this.selection.type !== "clip")
        return;
  ...
  // selectClip 메서드 추가
  selectClip(sprite, track) {
    // 이전 클립 선택 해제
    const previousClip = document.querySelector(".animation-sprite.selected");
    if (previousClip) {
      previousClip.classList.remove("selected");
    }

    // 키프레임 선택 해제
    const previousKeyframe = document.querySelector(".keyframe.selected");
    if (previousKeyframe) {
      previousKeyframe.classList.remove("selected");
    }
    ...
    console.log("Clip selected:", this.selection);
  }
}
