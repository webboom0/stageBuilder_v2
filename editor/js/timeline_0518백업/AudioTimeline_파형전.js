...
export class AudioTimeline extends BaseTimeline {
  ...
  initAudioTracks() {
    console.log("현재 트랙 수:", this.tracks.size); // 디버깅용
    ...
    const audioElement = new Audio("../files/music/SUJESHUN.mp3");

    audioElement.addEventListener("loadedmetadata", () => {
      const audioObject = new THREE.Object3D();
      audioObject.name = "SUJESHUN";
      ...
      // 초기 클립 크기를 3분으로 제한
      const initialDuration = Math.min(MAX_DURATION, audioElement.duration);
      const effectiveDuration = Math.max(MIN_DURATION, initialDuration);
      ...
      // 트랙 생성
      this.addTrack(audioObject.id, trackData);
    });
  ...
  createTrackElement(duration) {
    ...
    trackHeader.innerHTML = `
      <div class="track-info">
        <span class="track-name">SUJESHUN</span>
      </div>
    `;
    ...
    sprite.dataset.maxWidth = (180 / this.options.totalSeconds) * 100; // 3분으로 변경

    sprite.innerHTML = `
      <div class="sprite-handle left"></div>
      <div class="sprite-content">
        <span class="sprite-name">SUJESHUN</span>
      </div>
