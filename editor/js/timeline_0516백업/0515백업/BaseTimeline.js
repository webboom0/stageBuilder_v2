export class BaseTimeline {
  ...
  deleteKeyframe(objectId, propertyType, frame, keyframeElement) {
    // objectId가 객체인 경우 uuid 속성 사용
    const actualObjectId =
      typeof objectId === "object" ? objectId.uuid : objectId;
    ...
    if (
      ...
      console.log("선택된 키프레임 해제됨");
    }

    // 씬 업데이트
    if (this.editor.signals?.objectChanged) {
      const object = this.editor.scene.getObjectById(parseInt(actualObjectId));
      if (object) {
