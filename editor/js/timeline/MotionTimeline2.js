...
export class MotionTimeline extends BaseTimeline {
  ...
  addKeyframe(objectId, propertyType, frame) {
    ...
    console.log("키프레임 추가 상세 정보:", {
      ...
      timelineWidth: sprite.parentElement.offsetWidth,
    });
  ...
  duplicateClip(sprite, track) {
    // 원본 클립의 위치와 크기 가져오기
    const originalLeft = parseFloat(sprite.style.left) || 0;
    const originalWidth = parseFloat(sprite.style.width) || 100;
    ...
    // 새 클립을 트랙에 추가
    sprite.parentElement.appendChild(newSprite);
