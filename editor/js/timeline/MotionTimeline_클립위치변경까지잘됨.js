...
export class MotionTimeline extends BaseTimeline {
  ...
  addKeyframe(objectId, propertyType, frame) {
    ...
    // frame이 없으면 현재 playhead 위치의 프레임 계산
    if (frame === undefined) {
      frame = Math.floor(this.currentTime * this.options.framesPerSecond);
    }
    ...
    // 키프레임 데이터를 타임라인 기준 시간으로 저장
    track.keyframes[propertyType].set(frame.toString(), {
      value: value,
      time: timelineTime, // 타임라인 기준 절대 시간으로 저장
    });
    ...
    keyframeElement.dataset.time = timelineTime.toFixed(2);
    keyframeElement.dataset.position = JSON.stringify([
      value.x,
      value.y,
      value.z,
    ]);
    keyframeElement.dataset.pixelPosition = Math.round(
      this.lerp(0, 100, timelineTime / this.options.totalSeconds)
    ).toString();
    ...
    console.log("키프레임 추가 상세 정보:", {
      ...
      timelineWidth: clipElement.parentElement.offsetWidth,
    });
        ...
        ["position"].forEach((propertyType) => {
          ...
          const keyframeArray = Array.from(keyframes.entries())
            .map(([frame, data]) => ({
              ...
              time: data.time, // 이미 타임라인 기준 시간으로 저장되어 있음
              value: data.value,
            }))
