...
export class MotionTimeline extends BaseTimeline {
  ...
  addTrack(objectUuid, objectId, objectName) {
    console.log("=== addTrack 시작 ===");
    console.log("addTrack called with:", { objectUuid, objectId, objectName });
    console.log("현재 tracks:", this.tracks);
    console.log("container:", this.container);

    // 기존 트랙이 있지만 element가 null인 경우 새로운 트랙 생성
    if (this.tracks.has(objectId)) {
      const existingTrack = this.tracks.get(objectId);
      console.log("기존 트랙 확인:", existingTrack);

      if (existingTrack && existingTrack.element) {
        console.log("이미 존재하는 트랙입니다:", objectId);
        return existingTrack;
      } else {
        console.log("기존 트랙이 있지만 element가 null입니다. 새로운 트랙을 생성합니다.");
        // 기존 트랙 제거
        this.tracks.delete(objectId);
        this.tracksByUuid.delete(objectUuid);
      }
    }
    ...
    motionTracks.dataset.uuid = objectUuid;
    motionTracks.dataset.objectId = objectId;
    motionTracks.dataset.objectName = objectName;
    ...
    track.element.appendChild(motionTracks);

    // 트랙 객체 생성
    const object = this.editor.scene.getObjectByProperty('uuid', objectUuid);
    console.log("찾은 객체:", object);

    if (object && object.animations && object.animations.length > 0) {
      console.log("애니메이션이 있는 객체입니다");
      // FBX 애니메이션 mixer 초기화
    ...
    console.log("컨테이너에 트랙 추가 중...");
    console.log("container:", this.container);
    console.log("track.element:", track.element);

    if (this.container) {
      // timeline-container 찾기
      let timelineContainer = this.container.querySelector('.timeline-container');
      console.log("첫 번째 시도 - timeline-container:", timelineContainer);

      if (!timelineContainer) {
        // timeline-group.active 찾기
        const timelineGroup = this.container.querySelector('.timeline-group.active');
        console.log("timeline-group.active:", timelineGroup);

        if (timelineGroup) {
          timelineContainer = timelineGroup.querySelector('.timeline-container');
          console.log("두 번째 시도 - timeline-group.active에서 timeline-container:", timelineContainer);
        }
      }

      if (!timelineContainer) {
        // main-timeline-container 찾기
        const mainTimelineContainer = this.container.querySelector('.main-timeline-container');
        console.log("main-timeline-container:", mainTimelineContainer);

        if (mainTimelineContainer) {
          timelineContainer = mainTimelineContainer.querySelector('.timeline-container');
          console.log("세 번째 시도 - main-timeline-container에서 timeline-container:", timelineContainer);
        }
      }

      if (!timelineContainer) {
        // 최후의 수단으로 컨테이너 자체 사용
        timelineContainer = this.container;
        console.log("최후의 수단 - 컨테이너 자체 사용:", timelineContainer);
      }

      console.log("최종 timelineContainer:", timelineContainer);

      if (timelineContainer) {
        timelineContainer.appendChild(track.element);
        console.log("트랙이 timeline-container에 추가되었습니다");

        // 추가된 트랙이 실제로 DOM에 있는지 확인
        const addedTrack = timelineContainer.querySelector(`[data-object-id="${objectId}"]`);
        console.log("DOM에서 확인된 추가된 트랙:", addedTrack);
      } else {
        console.error("timeline-container를 찾을 수 없습니다!");
      }
    } else {
  ...
  onAfterLoad() {
    try {
      ...
      console.log("this.tracksByUuid:", this.tracksByUuid);
      ...
      if (this.editor.scene && this.editor.scene.userData) {
        ...
        if ((!this.timelineData || this.timelineData.tracks.size === 0) &&
          this.editor.scene.userData.motionTimeline) {
      ...
      if (!this.timelineData || this.timelineData.tracks.size === 0) {
        console.log("timelineData가 비어있으므로 트랙을 생성하지 않습니다.");
        console.log("scene.userData.motionTimeline:", this.editor.scene?.userData?.motionTimeline);
      }
  ...
  recreateUI() {
    try {
      console.log("=== recreateUI 시작 ===");
      console.log("container:", this.container);
      console.log("tracks:", this.tracks);
      console.log("timelineData.tracks:", this.timelineData?.tracks);

      // timelineData에서 tracks 복원
      if (this.timelineData && this.timelineData.tracks && this.tracks.size === 0) {
        console.log("timelineData에서 tracks 복원 중...");
        console.log("timelineData.tracks 크기:", this.timelineData.tracks.size);

        this.timelineData.tracks.forEach((objectTracks, objectUuid) => {
          console.log(`객체 ${objectUuid}의 트랙들:`, objectTracks);

          // 각 객체에 대해 기본 트랙 정보 생성
          const trackInfo = {
            uuid: objectUuid,
            name: "Object", // 기본 이름
            element: null
          ...
          this.tracks.set(objectId, trackInfo);
          this.tracksByUuid.set(objectUuid, trackInfo);
          console.log(`트랙 정보 추가: ${objectId} ->`, trackInfo);
        });
        console.log("tracks 복원 완료:", this.tracks);
      } else {
        console.log("timelineData가 없거나 tracks가 이미 존재합니다.");
        console.log("timelineData 존재:", !!this.timelineData);
        console.log("timelineData.tracks 존재:", !!(this.timelineData && this.timelineData.tracks));
        console.log("현재 tracks 크기:", this.tracks.size);

        // timelineData가 비어있으면 아무것도 하지 않음
        if (!this.timelineData || this.timelineData.tracks.size === 0) {
          console.log("timelineData가 비어있으므로 UI 재생성을 중단합니다.");
          return;
        }
      }

      // 기존 UI 정리 - 실제 구조에 맞게 수정
      let timelineContainer = this.container?.querySelector('.timeline-container');
      if (!timelineContainer) {
        // timeline-group.active 찾기
        const timelineGroup = this.container?.querySelector('.timeline-group.active');
      ...
      if (timelineContainer) {
        // 기존 트랙 요소들만 제거 (전체 내용 삭제하지 않음)
        const existingTracks = timelineContainer.querySelectorAll('.timeline-track');
        existingTracks.forEach(track => track.remove());
        console.log("기존 트랙 제거 완료:", existingTracks.length);
      }

      // tracks 데이터로부터 UI 재생성
      if (this.tracks && this.tracks.size > 0) {
        console.log("트랙 재생성 시작, 총 트랙 수:", this.tracks.size);

        // 현재 씬의 모든 객체 확인
        console.log("=== 현재 씬의 모든 객체 확인 ===");
        ...
        console.log("씬의 모든 객체:", allSceneObjects);

        this.tracks.forEach((track, objectId) => {
          try {
            console.log(`트랙 ${objectId} 재생성 중:`, track);

            // 1. UUID로 찾기
            let object = this.editor.scene.getObjectByProperty('uuid', track.uuid);
            console.log("UUID로 찾은 객체:", object);

            // 2. UUID로 찾지 못하면 이름으로 찾기
            if (!object && track.name) {
              object = this.editor.scene.getObjectByProperty('name', track.name);
              console.log("이름으로 찾은 객체:", object);
            }

            // 3. 여전히 찾지 못하면 씬의 모든 객체에서 비슷한 이름 찾기
            if (!object && track.name) {
              const similarObjects = allSceneObjects.filter(obj =>
                obj.name && obj.name.toLowerCase().includes(track.name.toLowerCase())
              );
              console.log("비슷한 이름의 객체들:", similarObjects);

              if (similarObjects.length > 0) {
                // 첫 번째 매칭되는 객체 사용
                object = this.editor.scene.getObjectByProperty('uuid', similarObjects[0].uuid);
                console.log("비슷한 이름으로 찾은 객체:", object);
              }
            }

            // 4. 여전히 찾지 못하면 씬의 모든 객체에서 타입으로 찾기
            if (!object) {
              const meshObjects = allSceneObjects.filter(obj =>
                obj.type === 'Group' || obj.type === 'Object3D' || obj.type === 'Mesh'
              );
              console.log("사용 가능한 메시 객체들:", meshObjects);

              if (meshObjects.length > 0) {
                // 첫 번째 사용 가능한 객체 사용
                object = this.editor.scene.getObjectByProperty('uuid', meshObjects[0].uuid);
                console.log("사용 가능한 객체로 대체:", object);
              }
            }

            if (object) {
              // addTrack 메서드 호출
              const newTrack = this.addTrack(object.uuid, objectId, track.name || object.name);
              console.log("새로 생성된 트랙:", newTrack);
            } else {
              console.warn(`객체를 찾을 수 없습니다: ${track.uuid}`);
              // 객체가 없어도 트랙은 생성해보자
              const newTrack = this.addTrack(track.uuid, objectId, track.name || "Unknown Object");
              console.log("객체 없이 생성된 트랙:", newTrack);
            }
          } catch (error) {
            console.warn(`트랙 ${objectId} 재생성 중 오류:`, error);
          }
        });
      } else {
        console.warn("재생성할 트랙이 없습니다.");
      }

      // 키프레임 UI 재생성
      if (this.timelineData && this.timelineData.tracks) {
        console.log("키프레임 재생성 시작");
        this.timelineData.tracks.forEach((objectTracks, objectUuid) => {
          try {
            const track = this.tracksByUuid.get(objectUuid);
            console.log(`객체 ${objectUuid}의 트랙:`, track);

            if (track && track.element) {
              objectTracks.forEach((trackData, property) => {
                try {
                  console.log(`속성 ${property} 재생성 중`);
                  // 속성 트랙 생성
                  this.createPropertyTrack(objectUuid, property);

                  // 키프레임들 재생성
                  const keyframes = trackData.getAllKeyframes();
                  console.log(`키프레임 수: ${keyframes?.length || 0}`);

                  if (keyframes) {
                    keyframes.forEach((keyframe, index) => {
                      try {
                        const sprite = track.element.querySelector('.animation-sprite');
                        console.log(`키프레임 ${index} 스프라이트:`, sprite);
                        ...
                        if (sprite && keyframe) {
                          ...
                          const keyframeLayer = sprite.querySelector('.keyframe-layer');
                          if (keyframeLayer && keyframeElement) {
                            keyframeLayer.appendChild(keyframeElement);
                            this.makeKeyframeDraggable(keyframeElement, track, keyframe.time, property);
                            console.log(`키프레임 ${index} 추가 완료`);
                          }
                        }
                      } catch (error) {
                        console.warn(`키프레임 ${index} 재생성 중 오류:`, error);
                      }
                    });
                  }
                } catch (error) {
                  console.warn(`속성 ${property} 재생성 중 오류:`, error);
                }
              });
            } else {
            ...
            console.warn(`객체 ${objectUuid} 재생성 중 오류:`, error);
          }
        });
      }

      console.log("=== recreateUI 완료 ===");
    } catch (error) {
      console.error("recreateUI 오류:", error);
    }
  }
    ...
    try {
      ...
      if (value) {
        if (Array.isArray(value)) {
          keyframeElement.dataset.value = JSON.stringify(value);
          keyframeElement.dataset.x = value[0]?.toString() || "0";
          keyframeElement.dataset.y = value[1]?.toString() || "0";
          keyframeElement.dataset.z = value[2]?.toString() || "0";
          ...
          keyframeElement.dataset.value = value.toString();
        }
      }
      ...
      console.error("키프레임 요소 생성 중 오류:", error);
      return null;
    }
  }
  ...
  onBeforeSave() {
    try {
      ...
      console.error("MotionTimeline onBeforeSave 오류:", error);
    }
  }
  ...
  saveTimelineData() {
    try {
      console.log("=== saveTimelineData 시작 ===");

      if (!this.timelineData) {
        console.warn("timelineData가 없습니다.");
        return;
      }

      console.log("현재 timelineData:", this.timelineData);
      console.log("timelineData.tracks 크기:", this.timelineData.tracks.size);

      // 현재 트랙 상태 저장
      this.tracks.forEach((track, objectId) => {
        if (track && track.uuid) {
          console.log(`트랙 ${objectId} (${track.uuid}) 저장 중...`);

          // 트랙 데이터가 없으면 생성
          if (!this.timelineData.tracks.has(track.uuid)) {
            this.timelineData.tracks.set(track.uuid, new Map());
            console.log(`새로운 트랙 데이터 생성: ${track.uuid}`);
          }
        }
      });
      ...
      if (this.editor && this.editor.scene) {
        ...
        // timelineData를 JSON으로 변환하여 저장
        const timelineDataJSON = this.timelineData.toJSON();
        this.editor.scene.userData.motionTimeline = timelineDataJSON;

        console.log("scene.userData.motionTimeline 저장 완료:", timelineDataJSON);
      } else {
        console.warn("editor 또는 scene이 없어서 userData에 저장할 수 없습니다.");
      }

      console.log("=== saveTimelineData 완료 ===");
    } catch (error) {
      console.error("타임라인 데이터 저장 중 오류:", error);
    }
  }
