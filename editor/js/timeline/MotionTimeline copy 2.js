...
export class MotionTimeline extends BaseTimeline {
  ...
  makeKeyframeDraggable(keyframeElement, track, frame, object) {
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      if (dy > deleteThreshold) {
        if (track && track.keyframes && track.keyframes["position"]) {
  ...
  updateAnimation(objectId) {
    ...
    // position 키프레임 처리
    if (track.keyframes.position) {
      const keyframes = Array.from(track.keyframes["position"].values()).sort(
        (a, b) => a.time - b.time
      );
      ...
      for (let i = 0; i < keyframes.length; i++) {
        if (keyframes[i].time <= currentRelativeTime) {
      ...
      if (prevKeyframe && nextKeyframe) {
        const alpha =
          (currentRelativeTime - prevKeyframe.time) /
