...
class Timeline {
  constructor(editor) {
    ...
    // 저장된 줌 레벨 로드
    const savedZoomLevel = localStorage.getItem('timelineZoomLevel');
    if (savedZoomLevel) {
      zoomSlider.value = savedZoomLevel;
      // 저장된 줌 레벨 적용
      const zoomLevel = parseFloat(savedZoomLevel);
      const basePixelPerSecond = 10;
      ...
      if (timeRuler) {
        timeRuler.style.width = `${newPixelPerSecond * this.timelineSettings.totalSeconds}px`;
      }

      tracks.forEach((track) => {
        track.style.width = `${newPixelPerSecond * this.timelineSettings.totalSeconds}px`;
      });
    } else {
      // 기본 줌 레벨 (1.0) 설정
      zoomSlider.value = "1";
    ...
    controlsContainer.appendChild(zoomSlider);  // 타임라인 줌 기능 활성화

    zoomSlider.addEventListener("input", (e) => {
      const zoomLevel = parseFloat(e.target.value);

      // 줌 레벨을 localStorage에 저장
      localStorage.setItem('timelineZoomLevel', zoomLevel.toString());
      ...
      tracks.forEach((track) => {
        ...
        clips.forEach((clip) => {
          // 클립의 % 위치와 크기는 그대로 유지
          // 줌은 트랙의 전체 너비만 변경하고, 클립의 상대적 위치는 유지
          const currentLeft = parseFloat(clip.style.left) || 0;
          const currentWidth = parseFloat(clip.style.width) || 100;
