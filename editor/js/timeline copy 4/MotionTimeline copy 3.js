...
export class MotionTimeline extends BaseTimeline {
  ...
  bindEvents() {
    this.container.addEventListener("click", (e) => {
      ...
      if (e.target.classList.contains("add-keyframe-btn")) {
        ...
        // 키프레임 추가
        const keyframeId = trackData.addKeyframe(timeInSeconds, position);
        if (!keyframeId) {
          console.warn("키프레임 추가 실패:", { timeInSeconds, position });
          return false;
        }

        // 최대 시간 업데이트
        this.timelineData.updateMaxTime(timeInSeconds);
        ...
        keyframeElement.dataset.keyframeId = keyframeId; // 키프레임 ID 설정
  ...
  addKeyframe(trackUuid, propertyType, timeInSeconds, value) {
    ...
    if (!track) {
      console.log("트랙이 없어서 생성합니다:", { trackUuid, propertyType });
      track = this.timelineData.addTrack(trackUuid, propertyType);
      if (!track) {
        console.error("트랙 생성 실패:", { trackUuid, propertyType });
        return false;
      }
    }

    // 키프레임 데이터 추가
    const keyframeId = track.addKeyframe(timeInSeconds, value);
    if (!keyframeId) {
  ...
  updateAnimation(time = null) {
    ...
    precomputedData.forEach((objectData, objectUuid) => {
      ...
      console.log("클립 기준 상대 시간:", relativeTime, "프레임 인덱스:", frameIndex);
  ...
  selectKeyframe(objectId, time, keyframeElement) {
    // 이전에 선택된 키프레임 제거
    ...
    const index = trackData.times.indexOf(time);
    console.log("index");
    console.log(index);
    if (index === -1) {
      console.warn("키프레임을 찾을 수 없습니다:", { time, objectId, property });
      return;
    }
    ...
    // 보간 방식 가져오기
    const interpolation = trackData.interpolations[index] || INTERPOLATION.LINEAR;
  ...
  addTrack(objectUuid, objectId, objectName) {
    ...
    if (object && object.animations && object.animations.length > 0) {
      ...
      // TimelineData에 트랙 생성 및 초기 키프레임 추가
      const trackData = this.timelineData.addTrack(objectUuid, 'position');
      const keyframeId = trackData.addKeyframe(0, position);
      if (keyframeId) {
        // 키프레임 UI 업데이트
        const keyframeElement = document.createElement('div');
        keyframeElement.className = 'keyframe';
  ...
  updateTrackUI(track, time) {
    ...
    for (let i = 0; i < trackData.keyframeCount; i++) {
      ...
      // 키프레임 위치 계산
      const position = (keyframeTime / this.timelineData.maxTime) * 100;
      keyframeElement.style.left = `${position}%`;
  ...
  makeKeyframeDraggable(keyframeElement, track, time, property) {
    ...
    keyframeElement.addEventListener("mousedown", (e) => {
      ...
      const handleMouseMove = (e) => {
        ...
        // 클립 밖으로 나갔는지 확인 (좌우)
        if (relativeX < 0 || relativeX > spriteRect.width) {
          if (!isOutsideClip) {
            isOutsideClip = true;
            keyframeElement.style.opacity = "0.5";
            keyframeElement.style.background = "#ff4444";
            console.log("키프레임이 클립 밖으로 나갔습니다 - 삭제 준비", {
              relativeX,
              spriteRectWidth: spriteRect.width,
              spriteRectLeft: spriteRect.left,
              clientX: e.clientX
            });
          }
        } else if (dy > REMOVE_THRESHOLD) {
          // 아래로 드래그해서 삭제하는 기존 기능도 유지
          if (!isOutsideClip) {
            isOutsideClip = true;
          ...
          if (track.uuid) {
            ...
            if (trackData) {
              ...
              const success = trackData.updateKeyframeTimeById(keyframeId, newTimeInSeconds);
              if (success) {
                // UI 업데이트
                ...
                console.warn("키프레임 시간 업데이트 실패:", keyframeId);
              }
            }
          }
        }
        ...
        if (isDragging) {
          ...
          if (isOutsideClip) {
            ...
            if (track.uuid) {
              ...
              if (trackData) {
                const keyframeId = keyframeElement.dataset.keyframeId;
                console.log("삭제할 키프레임 ID:", keyframeId, "속성:", property);
                const success = trackData.removeKeyframeById(keyframeId);
                if (success) {
                  console.log(`${property} 속성 키프레임 삭제됨:`, keyframeId);
                } else {
                  console.warn("키프레임 삭제 실패:", keyframeId);
                }
              } else {
  ...
  makePropertyKeyframeDraggable(keyframeElement, track, propertyType) {
    ...
    keyframeElement.addEventListener("mousedown", (e) => {
      ...
      const handleMouseMove = (e) => {
        ...
        //     keyframeElement.style.opacity = "0.5";
        //     keyframeElement.style.background = "#ff4444";
        //     keyframeElement.classList.remove("delete-preview"); // 아래 드래그 삭제 상태 해제
        //     console.log("키프레임이 클립 밖으로 나갔습니다 - 삭제 준비", {
        //       relativeX,
        //       spriteRectWidth: spriteRect.width,
        //       spriteRectLeft: spriteRect.left,
        ...
        if (isDragging) {
          ...
          if (isOutsideClip) {
            ...
            if (track.uuid) {
              ...
              if (trackData) {
                const keyframeId = keyframeElement.dataset.keyframeId;
                console.log("삭제할 키프레임 ID:", keyframeId, "속성:", propertyType);
                const success = trackData.removeKeyframeById(keyframeId);
                if (success) {
                  console.log(`${propertyType} 속성 키프레임 삭제됨:`, keyframeId);
                } else {
                  console.warn("키프레임 삭제 실패:", keyframeId);
                }
              } else {
              ...
              if (node.classList && node.classList.contains('keyframe')) {
                ...
                document.addEventListener("mousemove", (e) => {
                  ...
                  if (dy > deleteThreshold) {
                    node.remove();
                    if (track.keyframes && track.keyframes.has(propertyType)) {
                      track.keyframes.get(propertyType).delete(time.toString());
                    }
                    isDragging = false;
                    return;
                  }
                  ...
                  // 트랙 데이터 업데이트
                  if (track.keyframes && track.keyframes.has(propertyType)) {
                    const keyframeData = track.keyframes.get(propertyType).get(time.toString());
                    if (keyframeData) {
                      track.keyframes.get(propertyType).delete(time.toString());
