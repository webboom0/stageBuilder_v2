...
/** 오른쪽 패널: 오디오 객체만 표시 (userData.type === 'audio') */
function SidebarSceneAudio(editor) {
  ...
  function getObjectType(object) {
    if (object.userData && object.userData.type === "audio") return "Audio";
    return "Object3D";
  }
  ...
  function isAudioObject(object) {
    return object.userData && object.userData.type === "audio";
  }

  let ignoreObjectSelectedSignal = false;
  const outliner = new UIOutliner(editor);
  outliner.setId("outliner-audio");
  ...
  function refreshUI() {
    ...
    function addObjects(objects, pad) {
      for (let i = 0; i < objects.length; i++) {
        ...
        if (!isAudioObject(object)) continue;
