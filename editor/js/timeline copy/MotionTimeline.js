...
export class MotionTimeline extends BaseTimeline {
  ...
  play() {
    console.log("motionTimelien- play");
        ...
        if (dy > REMOVE_THRESHOLD) {
          ...
          if (track.uuid) {
            ...
            if (trackData) {
              ...
              console.log("전체 시간 배열:", Array.from(trackData.times.slice(0, trackData.keyframeCount)));
              console.log("전체 값 배열:", Array.from(trackData.values.slice(0, trackData.keyframeCount * 3)));

              // 키프레임의 시간만 직접 수정 (삭제/추가 방식 대신)
              trackData.times[dragStartIndex] = newTimeInSeconds;

              // 디버깅: timelineData.tracks 업데이트 확인
              console.log("=== timelineData.tracks 업데이트 확인 ===");
              console.log("업데이트 전 dragStartIndex:", dragStartIndex);
              console.log("업데이트 전 시간:", trackData.times[dragStartIndex]);
              console.log("새로운 시간:", newTimeInSeconds);
              console.log("전체 키프레임 시간 배열:", Array.from(trackData.times.slice(0, trackData.keyframeCount)));

              // 키프레임 정렬 (시간 순서 유지)
              trackData.sortKeyframes();

              console.log("정렬 후 전체 키프레임 시간 배열:", Array.from(trackData.times.slice(0, trackData.keyframeCount)));
              console.log("정렬 후 전체 값 배열:", Array.from(trackData.values.slice(0, trackData.keyframeCount * 3)));
              console.log("=== timelineData.tracks 업데이트 완료 ===");
              ...
              console.log("track.keyframes", track.keyframes);
